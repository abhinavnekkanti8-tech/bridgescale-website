...
/**
 * Remote EOR integration (Implementation Plan §P6.5/6.6/6.7).
 *
...
export class RemoteService implements EorPartnerService {
  ...
  async enroll(operatorOrgId: string, countryCode: string): Promise<EorPartnerEnrollment> {
    ...
    if (!partnerLiveMode()) {
      const partnerSideId = stubId('remote_stub');
      this.logger.warn(`[stub] Remote enrolment requested for ${operatorOrgId} (${countryCode}) -> ${partnerSideId}`);
      return {
        partner: this.partner,
        partnerSideId,
        status: EorEnrollmentStatus.PENDING,
        liveMode: false,
        trackingUrl: `https://remote.example/contractors/${partnerSideId}`,
      };
    }
    ...
    void countryCode;
    throw new Error('Remote live mode wired but SDK call not implemented yet.');
  }
  ...
  async getContractor(partnerSideId: string): Promise<EorContractor> {
    ...
    throw new Error('Remote contractor-status live call not implemented yet.');
  }

  async handleWebhook(rawBody: string, signature: string): Promise<{ ok: true; status?: EorEnrollmentStatus }> {
    if (!partnerLiveMode()) {
      this.logger.warn(`[stub] Remote webhook received (sig=${signature.slice(0, 12)}…)`);
      return { ok: true };
    }
    void rawBody;
    throw new Error('Remote webhook live handling not implemented yet.');
  }
}
