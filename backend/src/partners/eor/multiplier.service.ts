...
/**
 * Multiplier EOR integration (Implementation Plan §P6.5/6.6/6.7).
 *
 ...
 *   - Set multiplier_API_TOKEN (and any required org/workspace IDs)
 *   - Replace the stub blocks with the partner SDK calls
 *   - Wire up the webhook signature check at the controller level
 */
@Injectable()
export class MultiplierService implements EorPartnerService {
  private readonly logger = new Logger('MultiplierService');
  readonly partner = EorPartner.MULTIPLIER;
  ...
  async enroll(operatorOrgId: string, countryCode: string): Promise<EorPartnerEnrollment> {
    ...
    if (!partnerLiveMode()) {
      const partnerSideId = stubId('multiplier_stub');
      this.logger.warn(`[stub] Multiplier enrolment requested for ${operatorOrgId} (${countryCode}) -> ${partnerSideId}`);
      return {
        partner: this.partner,
        partnerSideId,
        status: EorEnrollmentStatus.PENDING,
        liveMode: false,
        trackingUrl: `https://multiplier.example/contractors/${partnerSideId}`,
      };
    }
    ...
    void countryCode;
    throw new Error('Multiplier live mode wired but SDK call not implemented yet.');
  }
  ...
  async getContractor(partnerSideId: string): Promise<EorContractor> {
    ...
    throw new Error('Multiplier contractor-status live call not implemented yet.');
  }

  async handleWebhook(rawBody: string, signature: string): Promise<{ ok: true; status?: EorEnrollmentStatus }> {
    if (!partnerLiveMode()) {
      this.logger.warn(`[stub] Multiplier webhook received (sig=${signature.slice(0, 12)}…)`);
      return { ok: true };
    }
    void rawBody;
    throw new Error('Multiplier webhook live handling not implemented yet.');
  }
}
